...
真实例子：`DocumentsMy GamesTerraria` 有 6.6 GB，其中 `tModLoaderMods` 3.2 GB 是模组本体而不是存档——这是 Ludusavi 官方清单指定整个文件夹导致的，不是判断错误。这种情况下正确的做法是在管理器里把它换成 `Worlds`、`Players` 几个子目录，或者单独给它提高上限。
